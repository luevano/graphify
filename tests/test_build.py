...
def _stub_merge_graph(tmp_path, mark_stub):
    ...
    # AST-shaped (source_location "L<n>") in BOTH variants so the tier-scoped
    # replace rule (#2333) is held constant and `_stub` is the only difference.
    stub = {"id": "autoloads_thing", "label": "thing.gd", "file_type": "code",
            "source_file": "autoloads/thing.gd", "source_location": "L1"}
    if mark_stub:
        stub["_stub"] = True
    chunk = {
        "nodes": [
